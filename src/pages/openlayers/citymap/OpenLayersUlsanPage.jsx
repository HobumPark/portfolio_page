...
import {fetchSkoreaGeoJsonFile} from '../api/district.js'

const ULSAN_CENTER = fromLonLat([129.311386, 35.538377]);

const OpenLayersUlsanPage = () => {
...
useEffect(() => {
  ...
  const initialize = async () => {
    ...
    // 비동기 API 호출
    const data = await fetchPollutionData("서울");
      ...
      .then((data) => {
        ...
        const seoulOnly = {
          ...
          features: data.features.filter(
            (f) => f.properties.code?.toString().startsWith('26')
          ),
        };
  ...
  const fetchGeoCenterJsonFile = () => {
    fetch('/data/ulsan_municipalities_geo_center.json')
      .then((response) => {
        if (!response.ok) throw new Error('네트워크 오류');
        return response.json();
      })
...
export default OpenLayersUlsanPage;
