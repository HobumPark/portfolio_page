...
useEffect(() => {
  ...
  mapInstanceRef.current.on('pointermove', (evt) => {
    ...
    if (feature && feature.getGeometry().getType() === 'Point') {
      ...
      if (pollutionInfo) {
        html += `
          <div>PM10: ${pollutionInfo.pm10Value} ㎍/㎥</div>
          <div>PM2.5: ${pollutionInfo.pm25Value} ㎍/㎥</div>
          <div>CO: ${pollutionInfo.coValue} ppm</div>
          <div>NO₂: ${pollutionInfo.no2Value} ppm</div>
          <div>O₃: ${pollutionInfo.o3Value} ppm</div>
          <div>SO₂: ${pollutionInfo.so2Value} ppm</div>
          <div>KHAI: ${pollutionInfo.khaiValue}</div>
        `;
      }
      ...
      .then((data) => {
        ...
        const seoulOnly = {
          ...
          features: data.features.filter(
            (f) => f.properties.code?.toString().startsWith('11')
          ),
        };
  ...
  const fetchGeoCenterJsonFile = () => {
    fetch('/data/seoul_municipalities_geo_center.json')
      .then((response) => {
        if (!response.ok) throw new Error('네트워크 오류');
        return response.json();
      })
